...
//    승 인 대기 목록 조회
    ...
    public List<?> getPendingList(@PathVariable String type, HttpSession session) {
...
//    승인 대기 중 상세 목록 조회
    ...
    public Optional<?> getPendingById(@PathVariable String type, @PathVariable Long id, HttpSession session) {
        if (!AdminCheckUtil.isAdmin(session)) {
            throw new RuntimeException("그라데이션 관리자만 접근 가능합니다.");
        }
...
//    승인 상태 변경(승인, 반려)
    ...
    public ResponseEntity<String> updateApprovalStatus(@PathVariable String type, @RequestBody Object dto, HttpSession session) {
        if (!AdminCheckUtil.isAdmin(session)) {
            throw new RuntimeException("그라데이션 관리자만 접근 가능합니다.");
        }
